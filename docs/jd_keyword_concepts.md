...
**SORA 實作細節：**  
`data/` 目錄被讀取後，每個分類文件中的規則或 bullet 會轉成 document，並附上 `rule_id`、`source`、`category`、`section` 等 metadata。`dummy_compliance.txt` 只保留為 legacy fallback。這讓 reviewer 可以引用具體規則，而不是只說「根據政策」。
...
RAG 應檢索到 `COMPLIANCE-001: Do not guarantee returns`。Reviewer 再把它轉成清楚回答：「不可以承諾或暗示固定回報，應改成風險中性的表述。」
...
**實務例子 B：**  
測試 reviewer node 時 mock RAG 回傳 `COMPLIANCE-001`，確認 final report 一定包含「Do not guarantee returns」和「not investment advice」。
...
**實務例子 B：**  
合規報告列出 `COMPLIANCE-001`，讓 compliance user 可以追蹤回答依據，而不是只能相信模型說法。
